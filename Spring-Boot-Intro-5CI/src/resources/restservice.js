/**
 * Erlaubt die Benutzerauthentifizierung und den Zugriff auf persistente 
 * Entities über das REST-API des Servers.
 */
app.service("RestService", function($http, $q, $window, $log) {
  ...
  // Verhindert, dass bei HTTP-Status 401 (unauthorized) automatisch der 
  // Anmeldedialog des Browsers erscheint
  $http.defaults.headers.common["X-Requested-With"] = "XMLHttpRequest";

  
  /**
   * Überprüft Name und Passwort beim Server und liefert ein Promise auf ein 
   * Objekt mit Benutzerinformationen. Wenn erfolgreich, werden Name und Passwort 
   * bei zukünftigen REST-Anfragen so lange zur Authentifizierung verwendet, bis 
   * abmelden() aufgerufen wird.
   * 
   * Um Name und Passwort zu überprüfen, erfolgt ein GET auf /profile. Auf dem 
   * Server darf dieser Pfad nur authentifizierten Benutzern zugänglich sein.
   */
  this.anmelden = function(name, passwort) {
    $log.debug("RestService: anmelden()", name);
    
    // Falls die Authentifizierung nicht erfolgreich ist, werden diese Daten vom
    // Fehler-Konstruktor wieder gelöscht
    $http.defaults.headers.common.Authorization = "Basic " + $window.btoa(name + ":" + passwort);
    $http.defaults.withCredentials = true;
    
    // GET auf einem Pfad versuchen, für den man authentifiziert sein muss.
    // Die Response-Daten werden ignoriert.
  ...
  /**
   * Entfernt die aktuellen Authentifizierungsdaten.
   */
  this.abmelden = function() {
    $log.debug("RestService: abmelden()");
